...
CREATE TABLE t_node_status
   (
      node_id NUMERIC(19,0),
      transaction_id NUMERIC(19,0),
      PRIMARY KEY (node_id)
   );
...
DELETE FROM alf_node_status WHERE node_id IN 
    (
        SELECT node_id FROM t_node_status
    )
...
UPDATE alf_node_status set protocol =
  (
    SELECT n.protocol FROM alf_node n WHERE n.id = node_id
  )
