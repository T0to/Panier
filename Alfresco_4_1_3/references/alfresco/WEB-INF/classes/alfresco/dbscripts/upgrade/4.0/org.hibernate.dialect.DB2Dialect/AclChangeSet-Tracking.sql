...
--ASSIGN:change_set_max_id=next_val
select case when max(id) is not null then max(id)+1 else 1 end as next_val from alf_acl_change_set;

-- Create table in temp form
CREATE TABLE t_alf_acl_change_set
(
    id BIGINT NOT NULL GENERATED BY DEFAULT AS IDENTITY (start with ${change_set_max_id}),
