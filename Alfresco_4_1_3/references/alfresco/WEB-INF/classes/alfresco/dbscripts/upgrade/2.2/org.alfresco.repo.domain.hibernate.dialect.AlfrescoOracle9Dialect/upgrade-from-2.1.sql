...
INSERT INTO alf_locale (id, locale_str)
   ...
   FROM (
     SELECT DISTINCT(ma.mkey)
        ...
        WHERE NOT EXISTS
        (
           SELECT 1 FROM alf_locale l
           WHERE ma.mkey = l.locale_str
        )
...
DELETE 
   ...
   WHERE EXISTS
   (
      SELECT 1 FROM t_del_attributes t WHERE t.id = alf_map_attribute_entries.map_id
   )
...
DELETE 
   ...
   WHERE EXISTS
   (
      SELECT 1 FROM t_del_attributes t WHERE  t.id = alf_list_attribute_entries.list_id
)
...
DELETE 
   ...
   WHERE EXISTS
(
      SELECT 1 from t_del_attributes t WHERE t.id = alf_attributes.id
   )
   AND alf_attributes.id >= ${LOWERBOUND} AND alf_attributes.id <= ${UPPERBOUND}
;
